      ...
      <el-main style="max-width: 1200px; margin: 0 auto;">
        ...
        <el-card v-loading="loading">
          <el-result
            ...
            icon="warning"
            title="暂无权限查看"
            sub-title="本文为「培训专区」内容，仅对已通过「批发与培训」产业报名审核的用户开放。"
          >
            <template #extra>
          ...
          <div v-else-if="article">
            ...
            <div class="article-cover">
              <el-image :src="resolveCultureCoverSrc(article)" fit="cover" style="width: 100%; max-height: 400px;"></el-image>
            </div>
...
export default {
  ...
  setup() {
      ...
      try {
        ...
        if (store.state.user.token) {
          api.browseHistory.record({
            ...
            image: resolveCultureCoverSrc(article.value)
    ...
    const handleLike = async () => {
      const skipMsg = article.value?.type === 5
      try {
        if (isLiked.value) {
